...
async function mostrarAnuncios() {
    ...
    let html = 
    '<table><tr><td>Nome</td><td>Descrição</td><td>Preço</td><td>Condição</td><td>Excluir</td><td>Editar</td><td>Carrinho</td></tr>';
    ...
    for(var i = 0; i < listaAnuncios.length; i++) {
        ...
        '<tr>' +
            ...
            '<td><a href="javascript:excluirAnuncio(\'' + anuncio.id + '\')">Excluir</a></td>' +
            '<td><a href="../anuncios/editarAnuncio.html?id=' + i + '">Editar</a></td>' +
            '<td><a href="../carrinho/carrinho.html?id=' + anuncio.id + '"><button>Add ao card</button></a></td>' +
        '</tr>';
